...
class GithubPersistence(TemplatePersister):
    ...
    def persist(self, output_path, destination):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.copy_output_to_repo(
                output_path=output_path, repo_path=temp_dir)
    ...
    def clone_repo(repo_name, clone_path):
        try:
            subprocess.run(
                ["gh", "repo", "clone", repo_name, clone_path], check=True)
        except subprocess.CalledProcessError as e:
    ...
    def commit_and_push(repo_path):
        try:
            ...
            subprocess.run(
                ["git", "commit", "-m", f"Pushing files in {repo_path}"], cwd=repo_path)
